		...
		while (run) {
			...
			case 2:
				...
				for (int i = 0; i < members.size(); i++) {
					...
					if (members.get(i).getMemberName().equals(searchName)) {
						System.out.println(members.get(i).getMembeNum() + members.get(i).getMemberName()
								+ members.get(i).getPoint());
					}
				}
				break;
			case 3:
				System.out.println("포인트 변경 회원 이름:");
				name = sc.nextLine();
				for (int i = 0; i < members.size(); i++) {
